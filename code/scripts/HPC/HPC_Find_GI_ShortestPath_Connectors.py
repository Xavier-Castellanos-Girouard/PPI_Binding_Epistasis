...
def main():
    ...
    ## Get arguments. Will be used in the 
    iter_num = sys.argv[1]
    ...
    ## Import network
    with open((main_path+f"data/Controls/DegPres_Randomized_GI_Networks/random_GI_network_{iter_num}_G.pickle"), "rb") as handle:
        GI_negEpsi_network_G = pickle.load(handle)
    
    with open(main_path + "data/GI_module_dict.pickle", "rb") as handle:
        GI_module_dict = pickle.load(handle)
    ...
    file_path = f"data/Controls/Random_GI_Shortest_Path_Connectors/GI_pairwise_module_shortest_paths_{iter_num}_dict.pickle"
