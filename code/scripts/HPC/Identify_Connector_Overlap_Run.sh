...
# Generate an array from 0 to 10. The python script will be run 10 times in parallel.
array=($(seq 0 1 99))

# Generate the arguments for the Python script
args=()
for numerator in "${array[@]}"; do
    arg=$((multiplier * 100 + numerator))
    args+=("$arg")
done

# Function to run the Python script
run_python_script() {
    python ~/scratch/scripts/HPC_Identify_Connector_Overlap.py "$1"
}
...
parallel -j 10 run_python_script ::: "${args[@]}"
