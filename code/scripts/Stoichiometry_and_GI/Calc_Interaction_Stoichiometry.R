...
Yeast_protein_abundances <- 
  readxl::read_xlsx("../data/ProteinAbundances/Abundances_copies_per_cell.xlsx")
...
processed_Data <- read.table("../data/Mass_Spectrometry_Interactomes/preprocessedProteinDataForCorrelation.txt", sep = "\t", header = TRUE)

# Import Yeast full uniprot database containing protein sequences. 
Uniprot_reference_sequences <- 
  read.fasta(file = "../data/ProteinSequences/uniprotkb_proteome_UP000002311_2023_11_30.fasta",
             seqtype = c("AA"))

## Import statistically significant interactions
Yeast_Interactome_Sig <- read.csv("../data/Mass_Spectrometry_Interactomes/The_Yeast_Interactome_Edges.csv", sep = ";")
...
Yeast_Interactome_Stoich <-
  Yeast_Interactome_Stoich %>%
  dplyr::select(source, target, source_copy_number, target_copy_number, Abundance_Stoichiometry, Interaction_Stoichiometry)

# Export
write.csv(Yeast_Interactome_Stoich, "../results/Stoichiometry_and_GI/Yeast_Interactome_Stoich.csv")
