...
PDB_2_Uniprot_DF <- read.csv("../data/Precomputed_data/PDB2Uniprot.csv", row.names = 1)
row.names(PDB_2_Uniprot_DF) <- NULL # Reset row names

# Import PDB bind PPI file with extracted information
pdb_kd_PPI_DF <- read.csv("../data/Precomputed_data/Kd_PDB_list.csv", row.names = 1)
row.names(pdb_kd_PPI_DF) <- NULL # Reset row names

# Import Uniprot to HGNC gene symbol conversion table
human_Uniprot2HGNC_DF <- read.csv("../data/ProteinSequences/uniprotkb_database_HGNC_2024_02_28.tsv", sep = "\t")
...
# Export
write.csv(pdb_kd_PPI_yeast_DF, "../results/Kd_and_GI/Yeast_Kd_literature.csv")
...
# Export
write.csv(human_pdb_kd_PPI_DF, "../results/Kd_and_GI/Human_Kd_literature.csv")
