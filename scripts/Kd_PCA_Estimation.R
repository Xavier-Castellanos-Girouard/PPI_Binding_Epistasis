...
Protein_Abundances_comp_DF <- read_xlsx(paste0(dir, "R_Calc_Kd_Benchmark/data/Abundances_copies_per_cell.xlsx"))
Protein_Abundances_comp_colnames <- Protein_Abundances_comp_DF[2,]

# Protein abundance data from Levy et al 2014
Protein_Abundance_PCA_DF <- read_xls(paste0(dir, "R_Calc_Kd_Benchmark/data/Abundace_Levy_2014.xls"))
...
Protein_Interaction_PCA <- read_xls(paste0(dir, "R_Calc_Kd_Benchmark/data/Tarassov_2008_PPI.xls"))

# List of PPIs from Tarassov et al. 2008 (excluding contaminants)
Tarassov_only_Int_DF <- read.csv(paste0(dir, "R_Calc_Kd_Benchmark/data/network_1.tsv"), 
                                 sep = "\t",
                                 header = FALSE)
...
ggplot(data = NULL,
       mapping = aes(y = Protein_Abundances_merged_DF$GROWTH.CYTO,
                     x = Protein_Abundances_merged_DF$log10_Median_molecules_per_cell)) +
  geom_point() +
  theme_bw() +
  xlab("Protein Abundance Compendium (log10)") +
  ylab("YFP-PCA Growth")

## Invert axes
ggplot(data = NULL,
       mapping = aes(x = Protein_Abundances_merged_DF$GROWTH.CYTO,
                     y = Protein_Abundances_merged_DF$log10_Median_molecules_per_cell)) +
  geom_point() +
  theme_bw() +
  ylab("Protein Abundance Compendium (log10)") +
  xlab("YFP-PCA Growth")
...
# plot linear model
ggplot(data = Protein_Abundances_merged_DF,
       mapping = aes(y = log10_Median_molecules_per_cell,
                     x = GROWTH.CYTO)) +
  geom_point() +
  theme_bw() +
  xlab("YFP-PCA Growth") +
  ylab("Protein Abundance Compendium (log10)") +
  stat_function(fun = function(x) lin_model$coefficients[2]*x+lin_model$coefficients[1],
                color = "red",
                linewidth = 1)
...
#write.csv(Protein_Interaction_PCA_filtered_final_kd, file = paste0(dir, "R_Calc_Kd_Benchmark/results/Estimated_Kd2_PCA_intensitiesNorm_LinFit_MeanCopyNumberUsed.csv"))
